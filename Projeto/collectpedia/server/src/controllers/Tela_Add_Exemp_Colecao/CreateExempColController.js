...
export class CreateExempColController {
    async handle(request, response) {
        let { vol, nomeSerie, email, nomeCol, est_conserv, edNum, data_aqui, data_lanc } = request.body;
        ...
        const insertAgrega = await prismaClient.agrega.create({
            data: {
               ...
               fk_exemplar_id: maxId_
            }
        })
        
        return response.json(
            {
                ExempAdicionados: addExemplar,
                ExempAgregados: insertAgrega
            });
    }
}